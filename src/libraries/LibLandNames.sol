...
library LibLandNames {
    bytes32 public constant LAND_NAMES_STORAGE_POSITION =
        keccak256(abi.encode(uint256(keccak256('games.laguna.LibLandNames')) - 1)) & ~bytes32(uint256(0xff));
    ...
    function landNameStorage() internal pure returns (LandNameStorage storage lns) {
    ...
    function _getRandomName() internal returns (uint256[3] memory) {
        ...
        return [
            ...
            ds.validMiddleNames[LibRNG.getRuntimeRNG(ds.validMiddleNames.length)],
