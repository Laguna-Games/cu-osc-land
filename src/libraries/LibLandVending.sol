...
library LibLandVending {
    bytes32 public constant LAND_VENDING_STORAGE_POSITION =
        keccak256(
            abi.encode(uint256(keccak256("games.laguna.LibLandVending")) - 1)
    ...
    function landVendingStorage()
        internal
        pure
        returns (LandVendingStorage storage lvs)
    {
    ...
    function setBeginningByLandTypeAndPhase(
        ...
        landVendingStorage().beginningByLandTypeAndPhase[landType][
            phase
        ] = beginning;
    }
    ...
    function getLandInventory()
        internal
        view
        returns (uint256[2][13] memory inventory)
    {
        LandVendingStorage storage lvs = landVendingStorage();
        for (uint8 i = 1; i <= 13; i++) {
            inventory[i - 1][0] =
                lvs.maxLandsByLandType[i] -
                lvs.mintedLandsByLandType[i];
            inventory[i - 1][1] = getCurrentPricingByLandType(i);
        }
        return inventory;
    }

    function getCurrentPricingByLandType(
        uint8 landType
        ...
        uint256 secondPhaseQuantity = lvs.secondPhaseQuantityByLandType[
            landType
        ];
        uint256 phase = 1;
        ...
        if (currentLandsByLandType >= secondPhaseQuantity) {
            phase = 3;
            currentLandsOnCurrentPhase =
                currentLandsByLandType -
                secondPhaseQuantity;
            phaseQuantity =
                lvs.maxLandsByLandType[landType] -
                secondPhaseQuantity;
    ...
    function beginKeystoneToLand(
        ...
        require(
            terminus.balanceOf(msg.sender, keystonePool) > 0,
            "LibLandVending: Player has no keystones left."
        );
        require(
        ...
        IERC20(LibResourceLocator.wethToken()).transferFrom(
            msg.sender,
            LibResourceLocator.gameBank(),
            marketPrice
        );
    ...
    function batchFinishMinting(
        ...
        require(
            ...
            "LibLandVending: Array lengths must match."
        );
    ...
    function setKeystonePoolIdByLandType(
        ...
        landVendingStorage().keystonePoolIdByLandType[
            landType
        ] = keystonePoolId;
    }
    ...
    function enforceLandTypeIsValid(uint8 landType) internal pure {
        require(
            landType > 0 && landType <= 13,
            "LibLandVending: invalid land type."
        );
    }
    ...
    function setLandVendingStartingIndexByLandType(
        ...
        landVendingStorage().landVendingStartingIndexesByLandType[
            landType
        ] = startIndex;
    }
    ...
    function getNextTokenIdByLandType(
        uint8 landType
    ) private view returns (uint256) {
        LandVendingStorage storage lvs = landVendingStorage();
        return
            lvs.landVendingStartingIndexesByLandType[landType] +
            lvs.mintedLandsByLandType[landType];
    }
    ...
    function mintNextLandVendingToken(
        uint8 landType,
        address to
        ...
        uint256 nextTokenId = lvs.landVendingStartingIndexesByLandType[
            landType
        ] + lvs.mintedLandsByLandType[landType];
        ...
        LibERC721.setTokenURI(
            nextTokenId,
            lvs.defaultTokenURIByLandType[landType]
        );
        return nextTokenId;
    }

    function getRarityByLandType(
        uint8 landType
    ...
    function getLandVendingStartingIndexByLandType(
        ...
        return
            landVendingStorage().landVendingStartingIndexesByLandType[landType];
    }
    ...
    function getBeginningByLandTypeAndPhase(
        ...
        return
            landVendingStorage().beginningByLandTypeAndPhase[landType][phase];
    }
