...
import {LibDiamond} from '../../lib/@lagunagames/lg-diamond-template/src/libraries/LibDiamond.sol';
...
import {LibGasReturner} from '../../lib/@lagunagames/cu-common/src/libraries/LibGasReturner.sol';
...
contract AirlockFacet {
    ...
    function lockLandIntoGame(uint256 tokenId) external {
        ...
        emit LibEvents.LandLockedIntoGameV2(tokenId, LibERC721.erc721Storage().owners[tokenId], msg.sender);
        LibGasReturner.returnGasToUser('lockLandIntoGame', (availableGas - gasleft()), payable(msg.sender));
    }
    ...
    function unlockLandOutOfGame(uint256 tokenId, string memory tokenURI, uint256 _level) internal {
        ...
        emit LibEvents.LandUnlockedOutOfGameV2(tokenId, LibERC721.erc721Storage().owners[tokenId], msg.sender);
    }
    ...
    function unlockLandOutOfGameWithSignature(
        ...
        uint256 availableGas = gasleft();
        bytes32 hash = unlockLandOutOfGameGenerateMessageHash(tokenId, tokenURI, _level, requestId, blockDeadline);
