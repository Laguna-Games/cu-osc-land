...
import {ITestnetDebugRegistry} from '../../lib/@lagunagames/cu-common/src/interfaces/ITestnetDebugRegistry.sol';
...
contract LandFaucetFacet {
    function faucetMintLand(address to, uint8 landType) public {
        ITestnetDebugRegistry(LibResourceLocator.testnetDebugRegistry()).enforceDebugger(msg.sender);
        ...
        dna = LibLandDNA._setRarity(dna, LibLandVending.getRarityByLandType(landType));
        ...
        LibERC721.setTokenURI(nextTokenId, LibLandVending.landVendingStorage().defaultTokenURIByLandType[landType]);
        emit LibEvents.BeginLVMMinting(nextTokenId, LibLandNames._getFullName(nextTokenId), landType);
    }

    function faucetBatchMintLand(address to, uint8 landType, uint256 amount) external {
        ITestnetDebugRegistry(LibResourceLocator.testnetDebugRegistry()).enforceDebugger(msg.sender);
        LibLandVending.enforceLandTypeIsValid(landType);
